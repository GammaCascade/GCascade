...
specPlot[spec_]:=LogLogPlot[Interpolation[Thread[{energies,energies^2*spec}]][x],{x,0.1,10^12}];
...
GAttenuate[injSpectraPre_,zStart_,stepSize_]:=Quiet[Module[
{
...
result={},
strechedResult={},
stretchedEnergies =energiesGamma*(1+zCur[[zCurIndex]])/(1+(zCur[[zCurIndex]]-stepSize))
},
...
singleCycle[spectrum_,thiszCurReg_, thisStepSize_,curZ_]:=
Module[
{func,
 ...
 stretchedEnergies = energiesGamma*((1. + curZ)/(1. + (curZ - stepSize)))},
...
params=Table[{zCurReg[[i]],stepArray[[i]],zCur[[i]]},{i,1,Length[zCur] - 1}];
finalResult = Fold[singleCycle[#1,#2[[1]],#2[[2]],#2[[3]]]&, injSpectra, params];
Return[Evaluate[finalResult]]
];

GCascadePoint[injSpectraPre_,zStart_,stepSize_]:=Module[
{
injSpectra=injSpectraPre*10^9,
finalResult={},
luminosityLength=(1+zStart)*NIntegrate[c/(H0*Sqrt[\[CapitalOmega]\[CapitalLambda]+\[CapitalOmega]m((1+zVal)^3)]),{zVal,0,zStart},Method->{"Trapezoidal"},PrecisionGoal->2]
...
finalResult= ((1+zStart)^2*CosmoCascade[injSpectra,zStart,stepSize])/(4*\[Pi]*(luminosityLength*kpc*100)^2);
...
CosmoCascadeV2[injSpectra_,zStart_,zEnd_,stepSize_]:=Block[
{ zCur = Range[zStart, zEnd, -stepSize],
 ...
 singleCycle[spectrum_,thiszCurReg_, thisStepSize_,curZ_]:=
 Module[
{func, zRegionIndex = Position[zReg,thiszCurReg][[1,1]], attenuatedSpec, result, strechedResult, stretchedEnergies = energiesGamma*((1. + curZ)/(1. + (curZ - stepSize)))},
         attenuatedSpec = (extinctionCoeffs[[zRegionIndex]]^thisStepSize)*spectrum; 
         result = Total[(spectrum - attenuatedSpec)*diffSpectra[[zRegionIndex]]*dEnergiesGamma] + attenuatedSpec; 
         func = Interpolation[Thread[{energiesGamma, result}], InterpolationOrder -> 1]; 
         strechedResult = Table[func[i],{i, stretchedEnergies}]; 
...
GCascadeDiffuse[injSpectraPre_,zStart_,stepSize_,zFunction_]:=Block[
{injSpectra=injSpectraPre*10^9,finalResult={},zFunc={},volumeNorms={},params={}},
...
params = Prepend[params,{volumeNorms[[1]],diffuseDistances[[1]],0}];
